...
for FILE in $DATADIR/* ; do
  ...
  jobID=$(qsub -t 1-$MAX -N Holovir_MG_blast -o $LOGDIR/$BASE/'blastp-${JOB_ID}.out' -e $LOGDIR/$BASE/'blastp-${JOB_ID}.err' $SCRIPTS/blastp.sh $CHUNKSDIR/$BASE $MARKERDB $BLASTDBTMPDIR $LOCKDIR | grep -o "[0-9]*$")
  qsub -hold_jid_ad $jobID -N Holovir_MG_concat -o $LOGDIR/$BASE/'checkconcat-${JOB_ID}.out' -e $LOGDIR/$BASE/'checkconcat-${JOB_ID}.err' $SCRIPTS/checkconcat.sh $CHUNKSDIR/$BASE $LOGDIR/$BASE $RESULTDIR
done
